...
1. M초마다 K개가 생성 -> M초가 되기 전에 0개임
2. while 로 하나하나 초를 세며 현재 초가 M의 배수일때 K개를 누적시키고 손님이 도착한 시간대면 붕어빵을 하나씩 줄이는 방법
너무 오래걸림 -> 배열로 바꿔서 한번에 입력하는 방식으로 바꿈
3. M스텝으로 붕어빵 생성, 손님 오는 시간에 -1되는 리스트, 붕어빵 누적을 리스트로 각각 정의해주는 방식
-> 0초에 K개 있는 상태로 시작해서 틀림 범위 바꿔줌!
-> 초를 돌리는 거보다 훨씬 빨라졌지만 여전히 런타임 오류
4. 손님 오는 시간에 -1 미리 해두고 붕어빵 생성과 누적을 한번의 for문으로 돌리는 방법 -> 성공!
